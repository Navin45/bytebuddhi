...
class MultiAgentOrchestrator:
    ...
    async def execute_task(
        ...
        # 8. Execute within concurrency semaphore & timeout
        consumed = 0
        attempts = 0
        ...
        while attempts < max_attempts:
            ...
            try:
                ...
                summary = (
                    answer[: self.config.max_summary_chars] if len(answer) > self.config.max_summary_chars else answer
                )
    ...
    async def execute_tasks(
        ...
        try:
            while len(results) < len(tasks):
                ...
                if cancellation_token and cancellation_token.is_set():
                    logger.info("Orchestration cancelled, stopping remaining tasks", orchestration_id=orchestration_id)
                ...
                for finished_task in done:
                    # Identify task_id for completed future
                    completed_tid = None
                    for tid, t_fut in list(running_tasks.items()):
                        if t_fut == finished_task:
                            completed_tid = tid
                    ...
                    if completed_tid:
                        ...
                        # Failure policy: critical failure aborts downstream
            ...
            # Ensure all in-flight tasks are cancelled upon exit
            for running_fut in running_tasks.values():
                if not running_fut.done():
                    running_fut.cancel()

        # Build aggregated outcome in declared task order
        ordered_results = {t.task_id: results[t.task_id] for t in tasks if t.task_id in results}
