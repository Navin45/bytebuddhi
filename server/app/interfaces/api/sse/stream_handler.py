...
class SSEStreamHandler:
    ...
    async def stream_response(
        ...
        try:
            ...
            if metadata:
                yield SSEStreamHandler.format_sse({"type": "metadata", "metadata": metadata}, event="metadata")

            # Stream content chunks
            async for chunk in content_iterator:
                if chunk:
                    yield SSEStreamHandler.format_sse({"type": "content", "content": chunk}, event="content")
                    # Small delay to prevent overwhelming the client
                    await asyncio.sleep(0.01)

            # Send done event
            yield SSEStreamHandler.format_sse({"type": "done"}, event="done")

        except Exception as e:
            logger.error("Error during SSE streaming", error=str(e))
            # Send error event
            yield SSEStreamHandler.format_sse({"type": "error", "error": str(e)}, event="error")
