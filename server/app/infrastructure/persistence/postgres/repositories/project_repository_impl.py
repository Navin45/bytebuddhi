...
class ProjectRepositoryImpl(ProjectRepository):
    ...
    async def get_by_name(self, user_id: UUID, name: str) -> Project | None:
        ...
        """
        result = await self.session.execute(
            select(ProjectModel).where(ProjectModel.user_id == user_id, ProjectModel.name == name)
        )
        project_model = result.scalar_one_or_none()
        return self._to_domain(project_model) if project_model else None
    ...
    async def exists_by_name(self, user_id: UUID, name: str) -> bool:
        ...
        """
        result = await self.session.execute(
            select(ProjectModel.id).where(ProjectModel.user_id == user_id, ProjectModel.name == name)
        )
