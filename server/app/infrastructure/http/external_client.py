            ...
            try:
                ...
                if resp.status_code == 429:
                    ...
                    if is_idempotent and attempts < self.max_retries:
                        ...
                        logger.warning("HTTP 429 rate limit received, backing off", provider=provider, wait=backoff)
                ...
                if resp.status_code >= 500:
                    if is_idempotent and attempts < self.max_retries:
                        ...
                        logger.warning("HTTP 5xx server error, retrying", provider=provider, status=resp.status_code)
                        await asyncio.sleep(backoff)
                        continue
                ...
                return ExternalResponseText(decoded_str, status_code=resp.status_code, headers=dict(resp.headers))
