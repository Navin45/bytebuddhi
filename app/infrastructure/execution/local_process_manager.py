...
class LocalProcessManager(ProcessManager):
    ...
    def __init__(self, tracer: Tracer | None = None, meter: Meter | None = None) -> None:
        ...
        self._executions_counter: Counter = self.meter.create_counter(
            MetricNames.PROCESS_EXECUTIONS_TOTAL,
            unit="1",
            description="Total OS process executions",
        )
        self._failures_counter: Counter = self.meter.create_counter(
            MetricNames.PROCESS_FAILURES_TOTAL,
            unit="1",
            description="Total OS process execution failures",
        )
        self._duration_hist: Histogram = self.meter.create_histogram(
            MetricNames.PROCESS_DURATION,
            unit="s",
    ...
    async def run(
        ...
        with self.tracer.start_as_current_span(
            SpanNames.PROCESS_EXECUTE,
            attributes={},
        ) as span:
            self._executions_counter.add(1)
            ...
            # Record telemetry metrics and span attributes
            self._duration_hist.record(result.duration_seconds)
            span.set_attribute(
                SpanAttributes.PROCESS_EXIT_CODE,
                result.exit_code if result.exit_code is not None else -1,
            )
            span.set_attribute(SpanAttributes.EXECUTION_STATUS, result.status.value)

            if result.status != ProcessStatus.COMPLETED or result.exit_code != 0:
                self._failures_counter.add(1, {"execution_status": result.status.value})
